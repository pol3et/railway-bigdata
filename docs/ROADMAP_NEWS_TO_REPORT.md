...
- **Everything fits 6 GB with headroom:** `bge-m3` ≈ **1.06 GB fp16** (568M, 1024-dim, **8192 max seq** — only candidate that embeds full bodies); Ollama Qwen-4B-q4 ≈ 3 GB. Only the two **GPU** residents must be sequential (they share the 6 GB VRAM); **CPU-hosted encoders run alongside the GPU LLM** (32 GB RAM).
...
- **Sequential-pass hygiene (VRAM-only — corrected 2026-06-25):** the scarce resource is the **6 GB VRAM**, NOT RAM — the box has **32 GB RAM** (~12 GB free under load). So unload/sequencing is needed ONLY between **GPU** residents (the Ollama LLM vs a GPU encoder): `ollama stop`/`OLLAMA_KEEP_ALIVE=0` → `ollama ps` empty → GPU embed pass → `nvidia-smi` idle. **Prioritize speed:** host the torch encoders **CPU-resident in RAM and keep them warm** (e5-base+XLM-R+lid ≈ 1.1 GB of 32 GB) — they never touch VRAM, so they need NO unload and can run **in parallel with the GPU LLM**, eliminating load/unload churn. No need to kill MCP servers for RAM.

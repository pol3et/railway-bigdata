...
- Committed manifest from the patch: `output/evidence/ksh-live-check-2026-06-22/manifest.json`
...
| KSH STADAT | live-ok | Committed manifest: `output/evidence/ksh-live-check-2026-06-22/manifest.json`. Current-code validation command landed 6 artifacts and 92,509 bytes under ignored `output/runtime/ksh-live-check-validation/`. | The earlier three seeds were reachable but mislabelled: `sza0010` is inland waterway goods, `sza0006` is road goods, and `sza0009` is rail freight, not passenger. The active seed set now uses six title-verified rail tables and rejects non-XLSX HTTP-200 bodies. | Wave 3: KSH XLSX -> `StatFact` using the table-specific plan below. |

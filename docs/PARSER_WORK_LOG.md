...
- Bounded live retry command run 2026-06-22:
  ...
  and `max_retries=1`, writing through `LocalBronzeLander`.
- Bounded live retry output: `status=partial`, `artifact_count=1`,
  `byte_count=11679`, `http_statuses=[429, 200]`; HU landed after retry, AT
...
- Scope note: this proves retry/backoff, `Retry-After` handling, DOC
  ...
  It also proves one current bounded recent GDELT artifact. It does not prove
  full HU+AT live coverage or any long historical backfill.
...
| GDELT recent | partial live-ok | Current-code manifest: `output/evidence/gdelt-live-check-2026-06-22/manifest.json` with 1 artifact, 11,679 bytes, and HTTP statuses `[429, 200]`. | Recent DOC API fetch now retries HTTP 429, respects `Retry-After`, and caps `maxrecords` at the documented 200-record bound. HU landed after retry; AT failed with a remote disconnect in the bounded probe. | Investigate AT retry behavior and add a first-class live-check collector if GDELT should be part of routine evidence runs. |
...
- Eurostat and World Bank still need Silver parsing work before they feed
  ...
  live evidence.
- Recent GDELT now has mocked retry coverage and partial bounded live evidence
  for HU; AT still needs live follow-up. Historical GDELT has mocked
...
| GDELT recent | `src/railway_lakehouse/bronze/sources/gdelt.py` | GDELT DOC 2.0 API | Recent rail-related news article lists for HU/AT source countries. | `bronze/news/gdelt/<geo>/ingest_date=YYYY-MM-DD/gdelt_doc_<geo>_<timespan>.json` plus `.meta.json`. | `silver/news/extract.py::gdelt_passthrough` or article extraction, then Gold news aggregation. | Query and mocked 429 retry behavior unit-tested; request size capped at 200 records. Bounded live retry landed HU after `[429, 200]`; AT failed with a remote disconnect. |

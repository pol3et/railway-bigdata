...
| GAP-006 | open | Workstream D - Silver Stats Owner / Workstream E - Silver News Owner | S3 local fixture E2E | Active branches split this gap: `silver/news-rss-article-records` owns the Silver News/RSS article-record half, and `silver/stats-worldbank-eurostat` owns the Silver Stats World Bank/Eurostat half. KSH XLSX -> `StatFact` parsing remains a later GAP-006 follow-up. | Silver reads Bronze fixtures/storage and writes auditable Silver outputs. KSH STADAT Bronze XLSX files become validated `StatFact` rows without LLM numeric rewriting. | Stats and news fixture inputs produce persisted Silver rows with unmapped labels/extraction failures visible; KSH XLSX fixture tests pass. | `python -m pytest -q -m integration` |

...
| GAP-005 | open | Workstream C - Bronze Owner | S2 data contracts | `src/railway_lakehouse/bronze/run.py` schedules Eurostat, World Bank, GDELT, and RSS only. KSH seeds are now corrected and live-validated, but KSH is still not scheduled. | KSH, Statistik Austria, UIC, and historical GDELT adapters are wired without changing raw landing semantics. | Scheduler imports and runs every configured source through the same `RawLander` contract. | `python -m pytest -q -m unit` |
...
| `python -m pytest -q tests\test_bronze_live_check.py` | passed: 8 passed | GAP-010 | Local KSH live-check collector handles `KshTable` seeds and rejects non-XLSX HTTP-200 bodies. |
| `python -m railway_lakehouse.bronze.live_check --sources ksh --out output/runtime/ksh-live-check-validation --max-artifacts 6 --timeout-seconds 30` | passed | GAP-010 | Current implementation landed 6 KSH artifacts, 92,509 bytes, 0 failures under ignored runtime output. |
